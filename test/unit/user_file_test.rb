...
class UserFileTest < ActiveSupport::TestCase
  ...
  test 'dependent share links get deleted' do
    ...
    assert_equal Boxroom::UserFile.all.count, 1

    3.times { create(:share_link, :user_file => file1) }
    assert_equal file1.share_links.count, 3

    file2 = create(:user_file)
    assert_equal Boxroom::UserFile.all.count, 2

    5.times { create(:share_link, :user_file => file2) }
    assert_equal file2.share_links.count, 5
    assert_equal Boxroom::ShareLink.all.count, 8

    file1.destroy
    assert_equal Boxroom::ShareLink.all.count, 5

    file2.destroy
    assert_equal Boxroom::ShareLink.all.count, 0
  end
  ...
  test 'folder is not empty' do
    file = Boxroom::UserFile.new(:attachment => fixture_file)
    assert !file.save
  ...
  test 'attachment file name is unique' do
    ...
    assert Boxroom::UserFile.exists?(:attachment_file_name => 'Test.txt')
    ...
    file3 = Boxroom::Folder.root.user_files.build(:attachment => fixture_file)
  ...
  test 'copy a file' do
    ...
    assert_raise(ActiveRecord::RecordInvalid) { file.copy(Boxroom::Folder.root) }
    assert_equal Boxroom::UserFile.all.count, 1
    ...
    new_file = Boxroom::UserFile.find_by_attachment_file_name_and_folder_id(file.attachment_file_name, folder.id)
