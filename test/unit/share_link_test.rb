...
class ShareLinkTest < ActiveSupport::TestCase
  ...
  test 'a link token gets generated' do
    ...
    share_link2 = Boxroom::ShareLink.new
    share_link2.user_file = create(:user_file)
    share_link2.user = create(:user)
  ...
  test 'active share links' do
    ...
    assert_equal Boxroom::ShareLink.active_share_links.count, 4
  end
  ...
  test 'the correct file is returned for a given link token' do
    ...
    assert_raise(NoMethodError) { Boxroom::ShareLink.file_for_token(random_token) }

    old_link_token = share_link2.link_token
    assert_raise(RuntimeError) { Boxroom::ShareLink.file_for_token(old_link_token) }

    valid_link_token = share_link1.link_token
    assert_equal Boxroom::ShareLink.file_for_token(valid_link_token), user_file
  end
end
